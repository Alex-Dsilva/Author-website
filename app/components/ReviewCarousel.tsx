...
// Rewritten completely in native R3F tags to guarantee rendering during SSR
...
function HandCard({ review, index, total, activeIndex }: HandProps) {
  ...
  // Shifted positioning so it fits perfectly on screen
  ...
  return (
    <a.group position={spring.position} rotation={spring.rotation} scale={spring.scale}>
      <group ref={visualRef}>
        ...
        {/* pointerEvents: 'none' stops Html from stealing clicks from your buttons */}
  ...
  return (
    <div style={{ width: "100%", minHeight: "600px", height: "75vh", position: "relative", overflow: "hidden", background: "transparent" }}>
      ...
      <Suspense fallback={null}>
        {/* Pulled camera back slightly so the cards fit gracefully */}
        <Canvas shadows camera={{ position: [0, 0.2, 9.5], fov: 45 }} style={{ width: "100%", height: "100%" }}>
          <Scene activeIndex={activeIndex} />
        </Canvas>
      </Suspense>

      {/* Navigation controls - Max zIndex so they are never blocked by 3D layers */}
